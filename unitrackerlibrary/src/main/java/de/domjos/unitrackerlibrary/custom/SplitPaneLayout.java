...
public class SplitPaneLayout extends ViewGroup {
    ...
    private void extractAttributes(Context context, AttributeSet attrs) {
        if (attrs != null) {
            try(TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SplitPaneLayout)) {
                mOrientation = a.getInt(R.styleable.SplitPaneLayout_orientation, 0);
                mSplitterSize = a.getDimensionPixelSize(R.styleable.SplitPaneLayout_splitterSize, 5);
    ...
    private int getMaxSplitterPosition() {
        return switch (mOrientation) {
            case ORIENTATION_HORIZONTAL -> getMeasuredWidth() - mPaneSizeMin;
            case ORIENTATION_VERTICAL -> getMeasuredHeight() - mPaneSizeMin;
            default -> 0;
        };
    }
